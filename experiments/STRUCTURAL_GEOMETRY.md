...
3. The initial tentative scope (both datasets × both radii × three geometries, 15 epochs) extrapolated from smoke to 5.24 hours before all overhead, exceeding the remaining budget. The explicit first screen is therefore **both datasets × three geometries, radius 2.0, 20 epochs**, validation at 5/10/15/20. Radius 1.5 remains supported but is not queued in this first screen. This gives longer matched training at one user-requested radius without an incomplete second-radius comparator group.
4. Before launching that six-job screen, require both complete SL profiles to finish and be backed up. Estimate SL cost from complete training/full-validation measurements, and control-model cost conservatively from their smoke measurements. Add 20% contingency plus 120 seconds; launch only if that fits the unchanged remaining budget. This gate cannot add jobs, extend time, or evaluate test. It is not large HPO or a convergence experiment.
